...
class Seq2Seq:
    ...
    def _create_rnn(self):
        with tf.name_scope('RNN_cell'):
            ...
            self.output, self.out_state = tf.nn.dynamic_rnn(self.cell,
                                                            inputs=self.one_hot_seq,
                                                            sequence_length=self.lenghts,
    ...
    def _create_placeholders(self, batch_size=None, window_size=None):
        with tf.name_scope('input_data'):
            self.seq = tf.placeholder(tf.int32, [batch_size, window_size], name='seq')
